...
def _load_manifiestos(df: pd.DataFrame, session: Session, maps: dict) -> int:
    ...
    if rows:
        stmt = pg_insert(Manifiesto).values(rows).on_conflict_do_nothing(index_elements=["manifiesto"])
        session.execute(stmt)
        session.commit()
    return len(rows)
...
def _load_pagos_conductor(df: pd.DataFrame, session: Session, resp_map: dict[str, int]) -> int:
    ...
    if rows:
        stmt = pg_insert(PagoConductor).values(rows).on_conflict_do_nothing(
...
def _load_facturacion(df: pd.DataFrame, session: Session, resp_map: dict[str, int]) -> int:
    ...
    if rows:
        stmt = pg_insert(Facturacion).values(rows).on_conflict_do_nothing(
            index_elements=["manifiesto_id"]
        )
        session.execute(stmt)
        session.commit()
