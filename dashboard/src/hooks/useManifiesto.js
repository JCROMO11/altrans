...
export function useManifiesto() {
  ...
  const create = async (form) => {
    ...
    const { error } = await supabase.rpc('guardar_digitador', {
      p_manifiesto:           Number(form.manifiesto),
      p_mes,
      p_año:                  año,
      p_periodo:              periodo,
    ...
    if (error) throw error
    return { manifiesto: Number(form.manifiesto) }
  }
  ...
  const updateSeguimiento = async (manifiesto_id, form) => {
    const { error } = await supabase.rpc('guardar_operativo', {
      ...
      p_ajuste_positivo_flete:      form.ajuste_positivo_flete ? Number(form.ajuste_positivo_flete) : null,
      p_ajuste_negativo_flete:      form.ajuste_negativo_flete ? Number(form.ajuste_negativo_flete) : null,
    })
    if (error) throw error
  }
  ...
  return { search, create, update, remove, updateSeguimiento, updateTesoreria, updateFacturacion }
}
